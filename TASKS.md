...
**阶段: ✅ v1.0.0 已部署上线 — main 锁定为稳定发布，后续开发在 dev 分支进行**

- 远端: https://github.com/StepaniaH/rate-lens
- 线上: 已部署到 VPS（Caddy 托管静态文件）
- `main` = `dev` = `v1.0.0` = `fe8d0a8`（首次发布，三者同步）

## 分支策略

| 分支 | 用途 | 状态 |
|------|------|------|
| `main` | 稳定发布分支，仅接受 dev 合并 / 打 tag | ✅ v1.0.0 @ `fe8d0a8`，已推送 + 已部署 |
| `dev` | 后续开发分支 | ✅ @ `fe8d0a8`（与 main 同步，待新开发） |
...
| 2026-07-08 | 🐛 修复主题对比度 bug：`--color-base` 在 `@theme inline` 中生成 `.text-base{color:...}` 覆盖了 Tailwind 的 `text-base` 字号工具类，导致输入框/结论文字渲染为页面背景色（不可见）。移除 `--color-base`，+回归守卫测试。README（英文主 + 中文）补全。测试 55 |
| 2026-07-08 | 🚀 v1.0.0 部署上线：VPS Caddy 托管静态文件（`npm ci && npm run build` → `rsync dist/` → Caddyfile 站点块）。main 锁定为稳定分支，后续开发转 dev。远端 main/dev/v1.0.0 同步 @ `fe8d0a8` |
